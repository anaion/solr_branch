...
mkdir -p $CLASSES_DIR
for DIR in `/bin/find $WORKSPACE -maxdepth 1`; do
...
#change cygwin-lke path to win path, maven needs it that way
WEBAPP_PROJECT_DIR="`echo $WEBAPP_PROJECT_DIR | sed -e 's/\/cygdrive\///'`"
WEBAPP_PROJECT_DIR="`echo $WEBAPP_PROJECT_DIR | sed -e 's/\//:\\\/'`"
WEBAPP_PROJECT_DIR="`echo $WEBAPP_PROJECT_DIR | sed -e 's/\//\\\/g'`"

if [ ! -f $LIB_TIMESTAMP -o ! -d $LIB_SRC -o $WEBAPP_PROJECT -nt $LIB_TIMESTAMP ]; then
echo maven -o -d $WEBAPP_PROJECT_DIR war:webapp
maven -o -d $WEBAPP_PROJECT_DIR war:webapp
    mkdir -p $LIB_DIR
    for LIB in `/bin/find $LIB_SRC -name \*.jar`; do
